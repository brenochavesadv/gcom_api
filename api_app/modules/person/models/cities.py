...
class Cities(db.Model):
    ...
    id = db.Column('municipios_id', db.Integer, primary_key=True)
    ...
    wikidata_id = db.Column('wikidata_id', db.String(15), nullable=True, comment='Wikidata ID for the city')

    relationships = {
        'state': relationship("States", backref="cities", lazy='joined')
    }
    ...
    def __repr__(self):
        return f"<Cities {self.id} - {self.name}>"  
    def __str__(self):
        return f"Cities(id={self.id}, name={self.name}, ibge_code={self.ibge_code}, state_id_fk={self.state_id_fk})"
